...
public class EchoingColor : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        redTileMaterial = Resources.Load<Material>("Materials/ChromaticPair/RedTile");
        blueTileMaterial = Resources.Load<Material>("Materials/ChromaticPair/BlueTile");
        yellowTileMaterial = Resources.Load<Material>("Materials/ChromaticPair/YellowTile");
    ...
    {
        // Randomly choose a sequence for the door patterns
        randomizedPattern.Clear();
        for (int i = 0; i < 3; i++)
        {
            int randomIndex = Random.Range(0, tileMaterials.Count);
            randomizedPattern.Add(tileMaterials[randomIndex]);
        }

        // Assign the materials to the patterns on the door
        pattern1Renderer.material = randomizedPattern[0];
        pattern2Renderer.material = randomizedPattern[1];
        pattern3Renderer.material = randomizedPattern[2];
    }
